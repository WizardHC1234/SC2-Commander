...
{{
  "strategy_contract":{{"style":"combat style","core_win_mechanism":"how the strategy wins","critical_power_window":"intended contact stage","core_commitments":["defining behavior"],"protected_invariants":["style or winning mechanism that evidence shows must remain"],"flexible_components":["producer count, composition detail, support, upgrade, gate, or reinforcement behavior that evidence may justify changing"]}},
...
Each package must state the one trajectory-level behavior it changes, the evidence connecting that behavior to losses, the winning behavior it preserves, and only the dependencies needed to realize the change. Account for production cost and post-contact continuation when they are causally relevant, but do not broaden a local fix into a full strategy redesign. Classify the direction by its observable behavioral effect rather than its unit, upgrade, threshold, or wording; adding a different pre-attack requirement is still the same delay-for-a-larger-package direction when it produces the same contact trajectory.

Only units and upgrades that must complete before the first commitment belong in timing_budget.package.gate_components. A support unit or upgrade is a parallel objective by default and must instead be described as non-blocking in plan.coordinated_changes. It may become a new hard gate only when hard_gate_evidence cites repeated trajectory evidence that waiting for it improves relative combat power at a still-useful contact window. Any unit-specific upgrade must have at least one beneficiary unit in the declared first-commitment package.
...
{{
  ...
  "candidate_packages":[
    {{
      ...
      "hard_gate_evidence":["Game N @ Ts: evidence that every newly blocking support unit or upgrade should delay the first commitment; empty when none is newly blocking"],
...
When a usable set exists, select exactly one package without combining or rewriting it. Preserve the Champion's repeated winning behavior and prefer the smallest evidence-supported causal change. Packages with status `unresolved`, `ineffective`, or `unsupported_hard_gate` are ineligible. Status `inherited_unresolved` means the timing simulator inherited an unresolved action from the Champion and the candidate introduced no new unresolved action; it remains eligible for full strategy validation and empirical match evaluation, but its timing must be treated as unknown. A `timing_risk` package requires direct trajectory evidence that its later contact remains useful; a self-declared time allowance is not evidence. {grounding_rule} Do not default to extra counter units, support, upgrades, scouting, defense, or higher gates. A newly blocking support unit or upgrade is valid only when the report includes hard-gate evidence and the observed opponent window remains favorable; otherwise keep it parallel and non-blocking. Reject any package that adds Bunkers or missile turrets, including entrance missile turrets. Compare every package semantically with prior experiments. A failed direction may receive at most one material repair that names a dependency which prevented realization; a second repair or a numerical restatement is repeats_failed and ineligible. Select inspect_runtime only when the supported problem is outside strategy control.

Before approving the selected package, perform an explicit direction audit. Identify the earliest supported root cause, describe what observably changes before contact, during engagement, and after contact, and assign a stable behavior-level semantic_family. Different units, upgrades, or thresholds belong to the same family when they produce the same trajectory. If the selected package repeats a rejected family, misses the supported root cause, or only delays contact without evidence of a better relative matchup, use regenerate_candidate_packages rather than approving it.
...
{{
  ...
  "direction_audit":{{"verdict":"approve_for_trial|revise_before_trial|reject_repeated_direction|inspect_runtime","root_cause":{{"primary":"production_readiness|attack_gate_timing|engagement_position|matchup_composition|post_defense_conversion|runtime_execution|mixed","reason":"earliest supported cause","evidence_refs":["Game N @ Ts"]}},"behavioral_change":{{"pre_contact":{{"changes_observable_trajectory":true,"description":"observable timing or production change","deadline_supported":true}},"engagement":{{"changes_observable_trajectory":false,"description":"matchup, target, or position change"}},"post_contact":{{"changes_observable_trajectory":false,"description":"continuation or conversion change"}}}},"history_comparison":{{"semantic_family":"stable behavior-level family","equivalent_rejected_experiment_ids":[],"why_new_or_repeated":"semantic comparison"}},"preserved_gain_experiment_ids":[],"blocking_reasons":[],"recommended_revision":{{"change":"smallest revision or empty","preserve":[],"avoid":[]}},"confidence":"high|medium|low"}},
