...
Options:
  ...
  --matches N             Champion batch size
  --candidate-matches N   Candidate evaluation games
  --concurrency N         并发
  --max-generations N     全局最大实验代数
  --max-total-generations N  同 --max-generations
  --pass-score X          Champion score >= this value counts as mastery
  --mastery-score-threshold X  同 --pass-score
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --pass-score|--mastery-score-threshold) MASTERY_SCORE_THRESHOLD="$2"; shift 2 ;;
...
echo "Candidate    : up to ${CANDIDATE_MATCHES} evaluation games"
