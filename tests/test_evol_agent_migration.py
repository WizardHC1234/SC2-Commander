...
from evol_agent.core.optimization_agent_loop import (
    _compact_contact_evidence,
    extract_final_cross_match_decision,
...
def test_strategy_contract_normalizes_legacy_checkpoints() -> None:
    ...
    assert contract == {
        ...
        "critical_power_window": "",
        "observed_winning_signature": "",
        "winning_evidence": [],
        "core_commitments": ["gather before attacking"],
        "protected_invariants": ["gather before attacking"],
        "flexible_components": [],
...
def test_optimizer_decision_keeps_strategy_intent_and_outcome_contrast() -> None:
    contract = {
        ...
        "core_win_mechanism": "attack before enemy technology matures",
        "critical_power_window": "first infantry mass",
        "protected_invariants": ["early commitment", "continuous reinforcement"],
    }
    contrast = {
        "winning_pattern": "early pressure converts before enemy tech",
        "preservation_rule": "do not delay the first attack",
    }
    analysis = BattleAnalysis(
        ...
        raw={
            ...
            "strengths_to_preserve": [
                {"pattern": "early commitment", "evidence": ["Game 1 @ 260s"]}
            ],
            "hypothesis": "repair the earliest loss shortfall",
            "plan": {"direction": "preserve the early pressure window"},
        },
    )

    decision = extract_final_cross_match_decision(analysis)

    assert decision["strategy_contract"] == contract
    assert decision["outcome_contrast"] == contrast
    assert decision["winning_mechanism"] == ""
    assert decision["wins_to_preserve"] == decision["strengths_to_preserve"]


def test_contact_evidence_keeps_retreat_policy_and_runtime_source() -> None:
    group = [
        "group_0",
        "main_force",
        20,
                        ...
                        "results": [
                            {
                                ...
                                "timeline_rows": [
                                    {"time_s": 430, "trigger": "auto_retreat_triggered", "groups": [group]}
                                ],
                            }
                        ]
                    }
    ...
    evidence = _compact_contact_evidence(raw)

    assert evidence[0]["groups"][0]["retreat_ratio"] == 0.7
    assert evidence[0]["groups"][0]["command_source"] == "auto_retreat"
    assert evidence[0]["groups"][0]["objective_status"] == "contested"


def test_discovery_identifies_strategy_core_before_ranking_failures() -> None:
    payload, error = _normalize_cross_match_discovery(
        {
            "strategy_contract": {
                "identity": "one-base infantry pressure",
                "style": "early pressure",
                "core_win_mechanism": "attack before advanced defenses mature",
                "critical_power_window": "first completed infantry force",
                "core_commitments": ["preserve early commitment"],
                "flexible_components": ["exact producer count"],
                "optimization_boundary": "do not turn it into a scaling strategy",
            },
            "strengths": [],
            "weaknesses": [],
            "unknowns": [],
    ...
    assert "advanced defenses" in payload["strategy_contract"]["core_win_mechanism"]
