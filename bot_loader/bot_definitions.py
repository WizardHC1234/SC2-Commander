...
class BotDefinitions:
    ...
    def add_dummies(
        ...
        def _make_commander(params):
            race_str = BotDefinitions.index_check(params, 0, "terran")
            race = races.get(race_str, Race.Terran)
            return Bot(race, CommanderBot(race_name=race_str))

        bot_dict["commander"] = (_make_commander, None)
        # Alias kept for old launch scripts.
        bot_dict["universal_llm"] = (_make_commander, None)
