...
- Tech and buildings have prerequisites (e.g. Factory needs Barracks; many units need add-ons or research). Missing one link blocks a whole line; build the chain, then use it. A producer without its required add-on or tech building cannot train the dependent unit.
...
- Compare every explicit strategy target with exact completed and pending evidence. Preserve exact scale: 1 Factory does not satisfy a target of 2, and a Factory does not prove a Factory Tech Lab exists. Absence from completed, under-construction, queue or technology evidence means zero.
...
- One macro tool per action with one positive integer absolute to_count; merge duplicates. Use to_count=1 for research and the resulting structure count for morphs.
...
- While infrastructure is still missing (Barracks/Factories/add-ons not ready), prefer supply_left_at_most, objective_status_became / destination_reached, or an explicit game_time_at_least a short time ahead — not an unreachable combat-unit gate. unit_count predicates on structures (CommandCenter, Refinery, Barracks, ...) are always unreachable because no train_* action produces them; track construction progress with game_time_at_least or supply_left_at_most instead.
